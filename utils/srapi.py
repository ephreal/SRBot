...
# Kinda implements the API for my shadowrun site. Not really, but kinda.
...
async def get_character(name):
    """
    Fetches a character from https://shadowrun.needs.management

    name: string
        -> character: json
    """

    url = "https://shadowrun.needs.management/api/character"
    if name:
        url += f"/name/{name}"
...
async def format_character(character_json):
    """
    Formats a character for discord.

    character: String
        -> discord.Embed
    """

    content = Embed(title=f"{character_json['name']}")
    content.set_footer(text=f"Player: {character_json['player']}")
    content.description = character_json['description']
    content.colour = Colour.lighter_grey()
